...
def matrixMultiply(rows, cols):
    ...
    and B
    """
    matrix_a = generateMatrix(int(rows), int(cols))
    matrix_b = generateMatrix(int(rows), int(cols))

    #generate an matrix of zeros for matrix c
    matrix_c = np.zeros(int(rows) * int(cols))
    matrix_c = matrix_c.reshape(int(rows), int(cols)) #reshape to get rows x cols matrix
    ...
    if rows == cols:
        ...
        print("Program assumes a square matrix please re-run ")
